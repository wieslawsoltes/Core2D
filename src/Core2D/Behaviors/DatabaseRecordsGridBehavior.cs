...
public sealed class DatabaseRecordsGridBehavior : Behavior<DataGrid>
{
        ...
        for (var i = 0; i < _viewModel.Columns.Length; i++)
        {
            ...
            var options = new DataGridColumnDefinitionOptions
            {
                ...
                SearchTextProvider = item => item is RecordViewModel record
                    ? GetRecordValue(record, columnIndex) ?? string.Empty : string.Empty
            };
            var definition = new DataGridTextColumnDefinition
            {
                Header = column.Name,
                Binding = DatabaseColumnBindingFactory.CreateRecordValue(columnIndex),
    ...
    {
        foreach (var column in _columnLookup.Keys) column.PropertyChanged -= OnColumnPropertyChanged;
        _columnLookup.Clear();
        _columnDefinitions.Clear();
    }
    ...
    {
        if (sender is not ColumnViewModel column || !_columnLookup.TryGetValue(column, out var definition)) return;
        if (e.PropertyName == nameof(ColumnViewModel.Name)) definition.Header = column.Name;
        else if (e.PropertyName == nameof(ColumnViewModel.IsVisible)) definition.IsVisible = column.IsVisible;
    }
    ...
    private void UpdateFilterAndSearch(string? query)
    {
        ...
        Dispatcher.UIThread.Post(() =>
        {
            ...
            var filterDescriptor = new FilteringDescriptor(GlobalFilterKey, FilteringOperator.Custom,
                string.Empty, query, Array.Empty<object>(), item => MatchesRecord(item as RecordViewModel, query),
    ...
    {
        if (record is null || record.Values.IsDefault) return false;
        foreach (var value in record.Values)
        {
            if (value is not null && !string.IsNullOrWhiteSpace(value.Content) &&
                value.Content.IndexOf(query, StringComparison.OrdinalIgnoreCase) >= 0) return true;
        }
        return false;
    }

    private static string? GetRecordValue(RecordViewModel record, int index) =>
        !record.Values.IsDefault && index >= 0 && index < record.Values.Length ? record.Values[index]?.Content : null;

    private static void SetRecordValue(RecordViewModel record, int index, string? value)
    {
        if (!record.Values.IsDefault && index >= 0 && index < record.Values.Length && record.Values[index] is { } field)
            field.Content = value;
    }
}
