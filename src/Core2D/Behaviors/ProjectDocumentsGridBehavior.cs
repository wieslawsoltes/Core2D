...
public sealed class ProjectDocumentsGridBehavior : Behavior<DataGrid>
{
    ...
    private ProjectContainerViewModel? _viewModel;
    private IDisposable? _hierarchySubscription;

    public ProjectDocumentsGridBehavior()
    {
    ...
    protected override void OnAttached()
    {
        ...
        UpdateViewModel(AssociatedObject.DataContext as ProjectContainerViewModel);
    }
    ...
    private void ConfigureGrid(DataGrid grid)
    {
        ...
        grid.AutoScrollToSelectedItem = true;
    ...
    private void UpdateViewModel(ProjectContainerViewModel? viewModel)
    {
    ...
    private void OnHierarchyChanged(object? sender, PropertyChangedEventArgs e)
    {
        ...
        if (e.PropertyName is nameof(DocumentContainerViewModel.Pages)
    ...
    private void RebuildItemsView()
    {
        ...
        _hierarchicalModel.SetRoots(_viewModel.Documents);
    ...
    private void RebuildColumns()
    {
        ...
        var definition = new DataGridHierarchicalColumnDefinition
        {
            ...
            Options = options,
            Indent = 16,
            CellTemplateKey = "ProjectDocumentsCellTemplate"
        };
    ...
    {
        return item is BaseShapeViewModel;
    }

    private static bool? GetExpanded(object item)
    {
        return item is BaseContainerViewModel container ? container.IsExpanded : null;
    }

    private static void SetExpanded(object item, bool value)
    {
        if (item is BaseContainerViewModel container)
        {
    ...
    {
        private readonly ProjectDocumentsGridBehavior _owner;

        public HierarchyObserver(ProjectDocumentsGridBehavior owner)
        {
            _owner = owner;
        }
