...
public sealed class DatabaseColumnsGridBehavior : Behavior<DataGrid>
{
    ...
    private void UpdateViewModel(DatabaseViewModel? viewModel)
    {
        ...
        if (_viewModel is null) { ClearColumnsView(); return; }
    ...
    {
        if (_viewModel is null || AssociatedObject is null) return;
    ...
    {
        _columnsView = null;
        if (AssociatedObject is { }) AssociatedObject.ItemsSource = null;
    }

    private void EnsureColumnDefinitions()
    {
        if (_columnDefinitions.Count > 0) return;
        var nameAccessor = new DataGridColumnValueAccessor<ColumnViewModel, string?>(
            column => column.Name, (column, value) => column.Name = value ?? string.Empty);
        var nameOptions = new DataGridColumnDefinitionOptions
        {
            ...
            IsSearchable = true,
            SearchTextProvider = item => item is ColumnViewModel column ? column.Name ?? string.Empty : string.Empty
        };
        var nameDefinition = new DataGridTextColumnDefinition
        {
            Header = "Name",
            Binding = ColumnDefinitionBindingFactory.CreateBinding<ColumnViewModel, string?>(nameof(ColumnViewModel.Name),
                column => column.Name, (column, value) => column.Name = value ?? string.Empty),
            IsReadOnly = false,
        ...
        var visibleAccessor = new DataGridColumnValueAccessor<ColumnViewModel, bool>(
            column => column.IsVisible, (column, value) => column.IsVisible = value);
        var visibleOptions = new DataGridColumnDefinitionOptions
        {
            ...
            SearchTextProvider = item => item is ColumnViewModel column
                ? column.IsVisible.ToString(CultureInfo.InvariantCulture) : string.Empty
        };
        var visibleDefinition = new DataGridCheckBoxColumnDefinition
        {
            Header = "Visible",
            Binding = ColumnDefinitionBindingFactory.CreateBinding<ColumnViewModel, bool>(nameof(ColumnViewModel.IsVisible),
                column => column.IsVisible, (column, value) => column.IsVisible = value),
            IsReadOnly = false,
            Width = DataGridLength.Auto,
        ...
        var ownerAccessor = new DataGridColumnValueAccessor<ColumnViewModel, string?>(column => column.Owner?.Name, (_, _) => { });
        var ownerOptions = new DataGridColumnDefinitionOptions
        {
            ...
            IsSearchable = true,
            SearchTextProvider = item => item is ColumnViewModel column ? column.Owner?.Name ?? string.Empty : string.Empty
        };
        var ownerDefinition = new DataGridTextColumnDefinition
        {
            Header = "Owner",
            Binding = DatabaseColumnBindingFactory.CreateOwnerName(),
            IsReadOnly = true,
            Width = DataGridLength.Auto,
    ...
    private void UpdateFilterAndSearch(string? query)
    {
        ...
        Dispatcher.UIThread.Post(() =>
        {
            ...
            var filterDescriptor = new FilteringDescriptor(GlobalFilterKey, FilteringOperator.Custom,
                string.Empty, query, Array.Empty<object>(), item => MatchesColumn(item as ColumnViewModel, query),
                CultureInfo.CurrentCulture, StringComparison.OrdinalIgnoreCase);
            _filteringModel.SetOrUpdate(filterDescriptor);
            var searchDescriptor = new SearchDescriptor(query, SearchMatchMode.Contains, SearchTermCombineMode.Any,
                SearchScope.VisibleColumns, Array.Empty<object>(), StringComparison.OrdinalIgnoreCase,
                CultureInfo.CurrentCulture, wholeWord: false, normalizeWhitespace: true, ignoreDiacritics: true, allowEmpty: false);
    ...
    private static bool MatchesColumn(ColumnViewModel? column, string query)
    {
        ...
        if (!string.IsNullOrWhiteSpace(column.Name) && column.Name.IndexOf(query, StringComparison.OrdinalIgnoreCase) >= 0) return true;
        if (column.Owner is { } owner && !string.IsNullOrWhiteSpace(owner.Name) &&
            owner.Name.IndexOf(query, StringComparison.OrdinalIgnoreCase) >= 0) return true;
        return column.IsVisible.ToString(CultureInfo.InvariantCulture).IndexOf(query, StringComparison.OrdinalIgnoreCase) >= 0;
    }
}
