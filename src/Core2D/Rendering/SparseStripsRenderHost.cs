...
internal sealed class SparseStripsRenderHost : IRenderHost
{
    private SparseStripsRenderContext? _context;
    private WriteableBitmap? _bitmap;
    private Vector _dpi = new(96, 96);
    private PixelSize _pixelSize;

    public bool Supports(IShapeRenderer? renderer) => renderer is SparseStripsRenderer;
    ...
    public object BeginRender(IShapeRenderer renderer, DrawingContext drawingContext, PixelSize pixelSize, double scaling)
    {
        ...
        {
            throw new InvalidOperationException("Unable to create Sparse Strips render resources.");
        }
    ...
    public void EndRender(IShapeRenderer renderer, object renderContext, DrawingContext drawingContext, PixelSize pixelSize, Rect destination)
    {
        ...
        var context = (SparseStripsRenderContext)renderContext;
        context.RenderToPixmap();
        var span = context.GetPixelBytes();
    ...
    private bool EnsureResources(PixelSize pixelSize, double scaling)
    {
        ...
        {
            _context?.Dispose();
            _context = new SparseStripsRenderContext((ushort)pixelSize.Width, (ushort)pixelSize.Height);
