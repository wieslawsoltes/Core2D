...
public readonly record struct RulerScale(double First, double Step, int Count, int Subdivisions)
{
    ...
    public static RulerScale Create(double zoom, double offset, double length, double spacing = 80, int subdivisions = 10)
    {
        ...
        while (minor > 1 && step * zoom / minor < 5) minor = minor > 5 ? 5 : minor > 2 ? 2 : 1;
    ...
    public string Format(double value, CultureInfo culture)
    {
        ...
        {
            int precision = Step > 0 ? (int)Math.Clamp(Math.Floor(Math.Log10(Math.Abs(value))) - Math.Floor(Math.Log10(Step)), 0, 15) : 3;
            return value.ToString(precision == 0 ? "0E+0" : "0." + new string('#', precision) + "E+0", culture);
        }
        return value.ToString(digits == 0 ? "0" : "0." + new string('#', digits), culture);
    }
}
