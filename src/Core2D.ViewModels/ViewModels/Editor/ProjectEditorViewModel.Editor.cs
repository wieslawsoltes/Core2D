...
public partial class ProjectEditorViewModel
{
    ...
    public void OnImportWmf(Stream stream)
    {
        var importer = ServiceProvider.GetService<IWmfImporter>();
        if (importer is null)
        {
            return;
        }
    ...
    public async Task<bool> OnDropFiles(IStorageItem[]? files, double x, double y)
    {
        try
        {
            ...
            foreach (var file in files)
            {
                ...
                else if (string.Compare(ext, ProjectEditorConfiguration.DefaultWmfExtension, StringComparison.OrdinalIgnoreCase) == 0)
                {
                    await using var stream = ServiceProvider.GetService<IFileSystem>()?.Open(path);
                    if (stream is { })
                    {
